...
public class CityCtrl extends SuperController<City, CityService, CityDtoQuery, CityDtoResponse> {
	...
	@RequestMapping("/tutu")
	public ResponseEntity<?> aze(){
		return ResponseEntity.ok().body(this.service.getAllStates());
	}
	
	@RequestMapping("/toto")
	public ResponseEntity<?> qsd(){
		return ResponseEntity.ok().body(this.service.getAllDepartment());
	}
